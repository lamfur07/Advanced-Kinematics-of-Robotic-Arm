...
ms=25; %markersize - the bigger the markersize the more covered the volume 
...
for i=q1  
    for j=q2   
       ...
       %Forward Kinematics (Write your forward kinematics for x, y and z)(MODIFY)
            x(count)=93.36*cos(theta1)+61.82*cos(theta1)*cos(theta2); 
            y(count)=-61.82*sin(theta2);
            z(count)=93.36*sin(theta1)+61.82*cos(theta2)*sin(theta1);
        end
    end
end
